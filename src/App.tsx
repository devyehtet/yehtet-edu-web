...
function LessonPreview() { return <div className="relative overflow-hidden rounded-2xl border border-white/15 bg-slate-100 shadow-2xl"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-4 py-3 text-[11px] text-slate-600"><div className="flex items-center gap-2"><span className="grid h-6 w-6 place-items-center rounded bg-emerald-500 text-white"><PlayCircle className="h-4 w-4" /></span><span className="font-bold text-slate-800">Vimeo Lesson Player - Why Digital Marketing Matters Today</span></div><span className="rounded-full bg-emerald-100 px-3 py-1 font-bold text-emerald-700">65% watched</span></div><div className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-4 sm:p-5"><div className="grid min-h-[220px] place-items-center rounded-xl border border-white/10 bg-[radial-gradient(circle_at_50%_30%,rgba(110,231,183,0.22),transparent_34%),linear-gradient(135deg,rgba(15,23,42,0.95),rgba(30,41,59,0.8))] sm:min-h-[300px]"><div className="text-center"><div className="mx-auto mb-5 grid h-20 w-20 place-items-center rounded-full border border-emerald-300/40 bg-emerald-300/10 text-emerald-300 shadow-2xl shadow-emerald-950/60 sm:h-24 sm:w-24"><PlayCircle className="h-10 w-10 sm:h-12 sm:w-12" /></div><h3 className="text-xl font-black text-white sm:text-2xl">Digital Marketing Beginner to Professional</h3><p className="mt-2 text-slate-300">Vimeo embed - skip protection enabled</p></div></div></div></div>; }
function ModuleCard({ module }: { module: (typeof modules)[number] }) { return <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-5 shadow-lg shadow-black/10"><p className={ui.eyebrow}>{module.title}</p><h3 className="mt-2 text-xl font-black text-white">{module.name}</h3><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-700"><div className="h-full rounded-full bg-emerald-300" style={{ width: `${module.progress}%` }} /></div><p className="mt-2 text-sm text-slate-400">{module.progress}% completed</p></div>; }
function LessonPlayerPage() { const [activeModuleIndex, setActiveModuleIndex] = useState(0); const activeModule = modules[activeModuleIndex]; return <section className="grid gap-6 lg:grid-cols-[1fr_410px]"><div className={ui.section}><div className="mb-5 flex flex-wrap items-start justify-between gap-4"><div><span className="inline-flex rounded-full border border-emerald-300/30 bg-emerald-300/10 px-4 py-1.5 text-xs font-black uppercase tracking-[0.32em] text-emerald-300">Student Lesson Player</span><h2 className="mt-2 font-serif text-4xl font-black leading-none sm:text-5xl md:text-6xl">Digital Marketing Beginner to Professional</h2></div><div className="flex items-center gap-2 pt-2 text-lg font-black text-white"><Users className="h-5 w-5 text-emerald-300" /> 5 joined live</div></div><LessonPreview /><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{modules.map((module,index) => <button key={module.title} onClick={() => setActiveModuleIndex(index)} className="text-left"><ModuleCard module={module} /></button>)}</div></div><aside className="space-y-6"><div className="rounded-[1.5rem] border border-white/10 bg-gradient-to-br from-[#1d2d63]/95 to-[#101538]/95 p-5 shadow-2xl shadow-black/30 sm:rounded-[1.75rem] sm:p-7"><p className={ui.eyebrow}>Selected Module</p><h2 className="mt-4 font-serif text-3xl font-black leading-[1.05] text-white sm:text-4xl">{activeModule.name}</h2><p className="mt-3 text-sm leading-6 text-slate-300">Open each lesson, watch the required progress, then complete the required task.</p><div className="mt-5 space-y-2">{activeModule.lessons.slice(0,6).map((lesson,index) => <button key={lesson} className="flex w-full items-center gap-3 rounded-xl border border-white/10 bg-white/[0.05] px-3 py-3 text-left text-sm text-slate-300"><PlayCircle className="h-4 w-4 shrink-0 text-emerald-300" />{String(index + 1).padStart(2,'0')}. {lesson}</button>)}</div></div></aside></section>; }
